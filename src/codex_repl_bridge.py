...
TELEGRAM_COMMAND_MENU = (
    {"command": "new", "description": "新建 Codex 对话会话"},
    {"command": "status", "description": "查看 Codex 状态与额度"},
    {"command": "context", "description": "查看当前上下文占用"},
    {"command": "model", "description": "选择模型与推理强度"},
    {"command": "permissions", "description": "查看或调整 Codex 权限"},
    {"command": "review", "description": "审查当前代码改动"},
    {"command": "compact", "description": "压缩当前对话上下文"},
    {"command": "help", "description": "查看 Codex 可用命令"},
    {"command": "ping", "description": "检查桥接是否在线"},
    {"command": "start", "description": "显示使用说明"},
    {"command": "init", "description": "生成或更新 AGENTS.md 指引"},
)
...
def codex_bridge_restart_command(node: str) -> list[str]:
    ...
    unit = os.environ.get("CODEX_BRIDGE_SYSTEMD_UNIT", "telegram-agent-bridge.service")
...
def format_long_running_progress_message(
    ...
    task_clause = f"任务 {task_id}：" if task_id else ""
    if recent:
        headline = f"✅ 仍在执行 · 最近进展：{recent}"
        body = (
            f"{task_clause}{label} 已执行约 {elapsed_minutes} 分钟。"
            f"最近进展：{recent}。完成后会自动发送最终结果。"
        )
    else:
        headline = "✅ 仍在执行 · 暂无可公开的进度"
        body = (
            f"{task_clause}{label} 已执行约 {elapsed_minutes} 分钟。"
            "正在等待 Codex 的最终结果。"
        )
...
class Config:
    ...
    def from_env(cls) -> "Config":
        ...
        return cls(
            ...
            auto_recover_tmux=bool_env("CRB_AUTO_RECOVER_TMUX", True),
...
class TelegramClient:
    ...
    def set_command_menu(self) -> bool:
        payload = self.call(
            "setMyCommands",
            _request_timeout=10,
            _attempts=1,
            _retry_delay=0,
            commands=json.dumps(TELEGRAM_COMMAND_MENU, ensure_ascii=False),
        )
    ...
    def send_restart_button(self, text: str, callback_data: str) -> bool:
        reply_markup = json.dumps(
            {"inline_keyboard": [[{"text": "\U0001f504 重启桥接服务", "callback_data": callback_data}]]},
            ensure_ascii=False,
        )
        payload = self.call(
            "sendMessage",
...
class TmuxTransport:
    ...
    def start_recovery_session(self) -> None:
        """Start the configured Codex TUI when its dedicated tmux server is gone."""
        session = self.config.tmux_session
        if session.startswith("%") or ":" in session or "." in session:
            raise RuntimeError(
                "automatic tmux recovery requires a bare session name; "
                f"configured target is {session!r}"
            )
        command = [
        ...
        if proc.returncode != 0:
            detail = (proc.stderr or proc.stdout or "").strip()
            # A concurrent recovery may have won the race; accept it only if
            # the target is now verifiably present.
            try:
                self.tmux("has-session", "-t", self.config.session_target)
                return
            except RuntimeError as exc:
                raise RuntimeError(f"tmux recovery start failed: {detail}") from exc

    def verify(self) -> None:
        try:
            self.tmux("has-session", "-t", self.config.session_target)
        except RuntimeError as exc:
            if not self.config.auto_recover_tmux:
                raise
            log("REPL", f"tmux target missing; starting recovery session: {exc}")
            self.start_recovery_session()
            self.tmux("has-session", "-t", self.config.session_target)
            log("REPL", f"tmux recovery session ready target={self.config.session_target}")
...
class Bridge:
    ...
    def handle_new_session_command(self) -> None:
        """Create a new Codex conversation and report the actual acknowledgement boundary."""
        try:
            previous_path = self.repl.session_file()
        except Exception as exc:  # noqa: BLE001
            log("REPL", f"new session preflight failed: {exc}")
            self.telegram.send(f"❌ 无法新建 Codex 对话：当前会话不可用：{exc}")
            return
        try:
            self.clear_and_paste_prompt("/new", "telegram new session")
        except Exception as exc:  # noqa: BLE001
            log("REPL", f"new session request failed: {exc}")
            self.telegram.send(f"❌ 新建 Codex 对话失败：{exc}")
            return

        # Codex creates the new JSONL lazily, on the first real prompt in the
        # new conversation. Therefore a path change is not a valid immediate
        # success condition. Check the TUI for a rejected command instead.
        time.sleep(0.8)
        if self.handle_slash_command_result("/new"):
            return
        try:
            current_path = self.repl.session_file()
        except Exception:
            current_path = previous_path
        if current_path != previous_path:
            log("REPL", f"new session confirmed path={current_path}")
            self.telegram.send("✅ 已新建 Codex 对话，后续消息将使用新的上下文。")
            return
        log("REPL", "new session accepted; JSONL will be created by the next prompt")
        self.telegram.send("✅ 已切换到新的 Codex 对话。请直接发送新的任务；新的会话日志会在该任务开始时创建。")
        ...
        if data.startswith(f"{BRIDGE_RESTART_CALLBACK_PREFIX}:"):
            ...
            if callback_query_id:
                self.telegram.call(
                    "answerCallbackQuery",
                    callback_query_id=callback_query_id,
                    text="正在重启 Codex 桥接服务…",
                )
            self.telegram.send("正在重启 Codex 桥接服务，稍后会自动恢复连接。")
            try:
                restart_codex_bridge(self.config.node)
            except Exception as exc:  # noqa: BLE001
                log("TG", f"bridge restart failed: {exc}")
                self.telegram.send(f"Codex 桥接服务重启失败：{exc}")
            return True
    ...
    def telegram_loop(self) -> None:
        ...
        while not self.stop_event.is_set():
            ...
            # The Telegram long-poll argument is not an end-to-end deadline:
            # a stalled HTTP read can otherwise hold this single consumer for
            # the client's generic 60-second timeout. Keep the transport
            # deadline close to the configured poll interval and let the next
            # loop recover unacknowledged updates.
            response = self.telegram.call(
                "getUpdates",
                _request_timeout=float(self.config.poll_timeout + 3),
                _attempts=1,
                _retry_delay=0,
                offset=offset,
                timeout=self.config.poll_timeout,
            )
            poll_elapsed_ms = round((time.monotonic() - poll_started_at) * 1000)
            self.record_poll_heartbeat()
            # getUpdates is configured as a short poll. A longer call queues
            # commands before their handlers can run.
            if poll_elapsed_ms > (self.config.poll_timeout + 3) * 1000:
                log(
                    "POLL",
                    f"getUpdates slow elapsed_ms={poll_elapsed_ms} "
                    f"configured_timeout_s={self.config.poll_timeout}",
                )
            if not response or not response.get("ok"):
                log("POLL", "getUpdates failed or timed out; retrying in 2s")
                time.sleep(2)
                continue
            for update in response.get("result", []):
                ...
                update_elapsed_ms = round((time.monotonic() - update_started_at) * 1000)
                if update_elapsed_ms > 2_000:
                    log("POLL", f"update handling slow update_id={update_id} elapsed_ms={update_elapsed_ms}")
    ...
    def process_telegram_update(self, update: dict[str, Any]) -> None:
        ...
        if prompt.image_path and self.config.image_mode == "exec":
            ...
            return
        command = prompt.text.strip().lower()
        if command == "/new":
            self.handle_new_session_command()
            return
        if command in {"/exit", "/quit"}:
            log("TG", f"blocked destructive REPL command={command}")
            self.telegram.send(
                "⚠️ 已拦截 /exit：它会关闭桥接依赖的 Codex tmux 会话。"
                "如需停止桥接，请在 VPS 执行 systemctl --user stop telegram-agent-bridge.service。"
            )
            return
        if command in {"/start", "/ping"}:
            # Health checks must expose the complete Telegram round trip.  This
            # command deliberately bypasses Codex, so its timing isolates the
            # bridge/API path from model and tmux latency.
            received_at = time.time()
            telegram_date = message.get("date")
            telegram_age_ms = "unknown"
            if isinstance(telegram_date, (int, float)):
                telegram_age_ms = str(max(0, round((received_at - telegram_date) * 1000)))
            update_id = update.get("update_id", "unknown")
            log(
                "PING",
                f"received command={command} update_id={update_id} "
                f"telegram_age_ms={telegram_age_ms}",
            )
            started_at = time.monotonic()
            log("PING", "sendMessage begin")
            sent = self.telegram.send("codex REPL bridge running")
            elapsed_ms = round((time.monotonic() - started_at) * 1000)
            log("PING", f"sendMessage {'success' if sent else 'failed'} elapsed_ms={elapsed_ms}")
            return
    ...
    def run(self) -> None:
        if self.telegram.set_command_menu():
            log("TG", f"command menu registered count={len(TELEGRAM_COMMAND_MENU)}")
        else:
